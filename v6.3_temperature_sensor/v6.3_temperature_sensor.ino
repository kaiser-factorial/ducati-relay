/*
  ...
  Hybrid BLE + outbound Wi-Fi version of the Lonely Binary ESP32-S3 sensor.
...
bool acknowledgeAction(
  ...
  if (measurement && measurement->valid) {
    ...
    result["celsius"] = roundf(measurement->temperatureC * 100.0f) / 100.0f;
    result["raw_adc"] = measurement->rawAdc;
...
void pollSphereAction() {
  ...
  if (actionType != "temperature_measurement") {
...
void setup() {
  ...
  delay(300);
  Serial.println("Ducati temperature sensor v6.3 ready.");
